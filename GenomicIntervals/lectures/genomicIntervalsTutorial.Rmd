...
Next, we are interested in average coverage per base for the promoters and later 
we are getting the maximum value of coverage per promoter using summary functions
that works on Views object.
```{r,viewMeans,fig_nums("rleview","Coverage vector extracted from RleList via Views() function is plotted as a line plot.")}
...
and other scores associated with genomic intervals. `genomation` package can 
...
```{r,fig.cap=fig_nums("karyoCpG","circos plot for CpG islands scores")}
